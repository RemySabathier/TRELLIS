...
class TrellisImageTo3DPipeline(Pipeline):
    ...
    def sample_sparse_structure(
        ...
        reso = flow_model.resolution  # [RS resolution == 16, in_channels == 8]
        # [RS -> (1, 8, 16, 16, 16)]
        noise = torch.randn(num_samples, flow_model.in_channels, reso, reso, reso).to(
            self.device
        )
        # [RS] {'steps': 25, 'cfg_strength': 5.0, 'cfg_interval': [0.5, 1.0], 'rescale_t': 3.0}
        ...
        # [RS] Note that decoder(z_s) -> (1,1,64,64,64) (values are unscaled)
        decoder = self.models["sparse_structure_decoder"]
        # [RS] -> (N_PTS, 4)
        # [RS] Where 4 is (MESH_ID | X | Y | Z), 0<=X<64
    ...
    def sample_slat(
        ...
        # [RS]: coords->(N_PTS, 4) feats->(N_PTS, 8)
        ...
        output = self.slat_sampler.sample(
            flow_model, noise, **cond, **sampler_params, verbose=True
        )
        slat = output.samples
    ...
    def run(
        ...
        # [RS] Generate the dino tokens for each cond. image
        cond = self.get_cond([image])
        torch.manual_seed(seed)
        # [RS] (N_verts,3): coords of the sparse structure in 64x64x64
        coords = self.sample_sparse_structure(
            cond, num_samples, sparse_structure_sampler_params
        )
        # [RS] (N_verts,8): structured latent in 64x64x64
