...
class SparseFeatures2Mesh:
    ...
    def __call__(self, cubefeats: SparseTensor, training=False):
        ...
        # [RS] self.res = 256
        # [RS] v_attrs_d->(257**3, 10) weights_d->(256**3, 21)
        ...
        vertices, faces, L_dev, colors = self.mesh_extractor(
            ...
            # Scalar field values at each vertex of the voxel grid.
            # Negative values denote that the corresponding vertex resides inside the isosurface.
            scalar_field=sdf_d,
            # Indices of 8 vertices for each cube in the voxel grid.
            cube_idx=self.reg_c,
            # ->256
            resolution=self.res,
            # (12) Weight parameters for the cube edges to adjust dual vertices positioning
            beta=weights_d[:, :12],
            # (8) Weight parameters for the cube corners to adjust dual vertices positioning
            alpha=weights_d[:, 12:20],
            # (1) Weight parameters to control the splitting of quadrilaterals into triangles.
